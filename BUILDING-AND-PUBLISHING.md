...
How to ship `retold-content-system` to npm and to GitHub Container Registry
(GHCR). Generated by `quack docker-init`; the structure matches the
shared template across all dockerized retold tools.

`retold-content-system` is a **long-running service**. Restart policy in compose / k8s should be `unless-stopped` (or equivalent). The Dockerfile should declare a HEALTHCHECK against the service's health endpoint.
...
BUILD_DOCKER=1 npm publish    (or: npm run publish:docker)
  ...
  → docker push ghcr.io/fable-retold/retold-content-system:<version>
```
...
3. **Image**: `docker pull ghcr.io/fable-retold/retold-content-system:latest`
...
```bash
docker pull ghcr.io/fable-retold/retold-content-system:latest
docker run --rm ghcr.io/fable-retold/retold-content-system:latest
```
